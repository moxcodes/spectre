...
SPECTRE_TEST_CASE("Unit.Evolution.Systems.Cce.PreSwshDerivatives",
  ...
  using swsh_derivatives_spare_variables_tag =
      ::Tags::Variables<tmpl::list<Spectral::Swsh::Tags::Derivative<
          Tags::BondiBeta, Spectral::Swsh::Tags::Eth>>>;
  auto spare_computation_box = db::create<db::AddSimpleTags<
      Spectral::Swsh::Tags::LMax, Spectral::Swsh::Tags::NumberOfRadialPoints,
      pre_swsh_spare_variables_tag, swsh_derivatives_spare_variables_tag>>(
      l_max, number_of_radial_grid_points,
      typename pre_swsh_spare_variables_tag::type{number_of_grid_points, 0.0},
      typename swsh_derivatives_spare_variables_tag::type{number_of_grid_points,
                                                          0.0});
  UniformCustomDistribution<double> dist(0.1, 1.0);
  tmpl::for_each<tmpl::list<Tags::BondiJ, Tags::BondiQ,
    ...
    &spare_computation_box, &generator, &dist
  ](auto tag_v) noexcept {
    using tag = typename decltype(tag_v)::type;
    db::mutate<tag>(
        make_not_null(&spare_computation_box),
        [&generator, &
         dist ](const gsl::not_null<db::item_type<tag>*> to_generate) noexcept {
