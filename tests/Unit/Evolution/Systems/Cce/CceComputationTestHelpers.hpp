...
// inputs to the integrand computation (`Test_ComputePreSwshDerivatives.cpp`,
...
void generate_separable_expected(
  ...
  tmpl::for_each<InputTagList>([
    ...
    for (size_t i = 0; i < radial_polynomial.size(); ++i) {
      radial_polynomial[i] *= exp(
          -10.0 * pow<2>(static_cast<double>(i) /
  ...
  // utilities
  auto calculate_separable_for_pre_swsh_derivative = [&angular_collocations,
                                                      &radial_modes,
                                                      &pre_swsh_derivatives,
                                                      &l_max,
                                                      &number_of_radial_points,
  ...
  auto calculate_separable_for_swsh_derivative = [&angular_collocations,
                                                  &radial_modes,
                                                  &swsh_derivatives, &l_max,
                                                  &number_of_radial_points,
                                                  &one_divided_by_r,
                                                  &boundary_r](auto x) {
    using swsh_derivative_tag = typename decltype(x)::type;
  ...
  tmpl::for_each<TargetTagList>(
      [&calculate_separable_for_pre_swsh_derivative,
       &calculate_separable_for_swsh_derivative](auto x) {
        using target_tag = typename decltype(x)::type;
        tmpl::for_each<pre_swsh_derivative_tags_to_compute_for<target_tag>>(
            calculate_separable_for_pre_swsh_derivative);
        tmpl::for_each<swsh_derivative_tags_to_compute_for<target_tag>>(
            calculate_separable_for_swsh_derivative);
        tmpl::for_each<second_swsh_derivative_tags_to_compute_for<target_tag>>(
            calculate_separable_for_swsh_derivative);
      });
}

// A utility for copying a set of tags from one data box to another. Useful in
