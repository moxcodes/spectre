...
struct SetBoundaryValues {
  ...
  static void apply(db::DataBox<tmpl::list<DbTags...>>& box,
                    Parallel::ConstGlobalCache<Metavariables>& /*cache*/,
                    const ArrayIndex& /*array_index*/,
                    const DataVector& set_values) noexcept {
    db::mutate<Tag>(
        make_not_null(&box), [&set_values](
                                 const gsl::not_null<db::item_type<Tag>*>
                                     scalar_quantity) noexcept {
          get(*scalar_quantity) = set_values;
        });
  }
};
...
struct mock_characteristic_evolution {
  ...
  using phase_dependent_action_list = tmpl::list<
      ...
      Parallel::PhaseActions<
          ...
          tmpl::list<
              Actions::EnqueueInterpolationScriData<Tags::News>,
...
SPECTRE_TEST_CASE("Unit.Evolution.Systems.Cce.Actions.ScriObserveInterpolated",
  ...
  ActionTesting::MockRuntimeSystem<test_metavariables> runner{
      tuples::tagged_tuple_from_typelist<
          ...
          filename, std::make_unique<::TimeSteppers::RungeKutta3>(), l_max,
          number_of_radial_points, scri_output_density, observation_l_max}};

  const double start_time = 0.0;
  const double end_time = 100.0;
  const double target_step_size = 3.0;
  ...
  const double linear_coefficient = value_dist(gen);
  const double quadratic_coefficient = value_dist(gen);
  ...
  for (size_t i = 0; i < 3 * data_points; ++i) {
    ...
    const DataVector time_vector =
        i + make_with_random_values<DataVector>(
  ...
  // scoped to close the file
  {
    ...
    auto cols = alg::iota(
        std::vector<size_t>(2 * square(observation_l_max + 1) + 1), 0_st);
    const Matrix data_matrix = dataset.get_data();
    CHECK(data_matrix.rows() > 9);
    auto expected_goldberg_modes =
