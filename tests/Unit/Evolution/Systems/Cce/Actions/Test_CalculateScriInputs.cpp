...
SPECTRE_TEST_CASE("Unit.Evolution.Systems.Cce.Actions.CalculateScriInputs",
  ...
  tmpl::for_each<all_real_boundary_scri_input_tags>([
    &component_real_variables, &component_volume_variables, &gen,
    &coefficient_distribution, &l_max
  ](auto tag_v) noexcept {
    using tag = typename decltype(tag_v)::type;
  ...
  auto expected_box = db::create<tmpl::append<
  ...
  tmpl::for_each<extra_pre_swsh_derivative_scri_tags>(
      [&expected_box, &runner ](auto tag_v) noexcept {
        using tag = typename decltype(tag_v)::type;
        db::mutate_apply<PreSwshDerivatives<tag>>(make_not_null(&expected_box));
      });

  tmpl::for_each<extra_precomputation_scri_tags>(
      [&expected_box, &runner ](auto tag_v) noexcept {
        using tag = typename decltype(tag_v)::type;
