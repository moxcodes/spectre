...
struct h5_metavariables {
  ...
  using component_list = tmpl::list<mock_h5_worldtube_boundary<h5_metavariables>>;
...
void test_h5_initialization(const gsl::not_null<Generator*> gen) noexcept {
  ...
  runner.set_phase(h5_metavariables::Phase::Initialization);
  ...
  ActionTesting::next_action<component>(make_not_null(&runner), 0);
  runner.set_phase(h5_metavariables::Phase::Evolve);
