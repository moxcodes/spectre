...
SPECTRE_TEST_CASE("Unit.Parallel.PhaseControl.PhaseChange",
  ...
  phase_change->initialize_phase_change_decision_data(
  ...
  const auto first_decision = phase_change->arbitrate_phase_control(
  ...
  const auto second_decision = phase_change->arbitrate_phase_control(
      make_not_null(&phase_change_decision_data), Metavariables::Phase::PhaseB,
      cache);
