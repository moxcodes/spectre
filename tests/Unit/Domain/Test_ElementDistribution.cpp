...
std::array<SegmentId, Dim> element_index_to_segment_id(
    const std::array<size_t, Dim>& refinements,
    const size_t element_index) noexcept {
  std::array<SegmentId, Dim> segment_ids{};
  for (size_t i = 0; i < Dim; ++i) {
    size_t stride = 1;
    for (size_t j = 0; j < i; ++j) {
      stride *= two_to_the(gsl::at(refinements, j));
    }
    const size_t element_index_in_dim =
        (element_index / stride) % two_to_the(gsl::at(refinements, i));
    segment_ids.at(i) =
        SegmentId{gsl::at(refinements, i), element_index_in_dim};
  }
  return segment_ids;
}
...
size_t number_of_elements_in_block(
    const std::array<size_t, Dim>& refinements) noexcept {
  size_t number_of_elements = 1;
  for (size_t i = 0; i < Dim; ++i) {
    number_of_elements *= two_to_the(gsl::at(refinements, i));
  }
...
std::vector<std::vector<size_t>> make_proc_map_for_domain(
  ...
  const domain::BlockZCurveProcDistribution distribution{number_of_procs,
                                                         refinements_by_block};
  for (size_t block = 0; block < number_of_blocks; ++block) {
    size_t number_of_elements_in_block = 1;
    for (size_t i = 0; i < Dim; ++i) {
      number_of_elements_in_block *=
          two_to_the(gsl::at(gsl::at(refinements_by_block, block), i));
    }
    ...
    for (size_t element_index = 0; element_index < number_of_elements_in_block;
      ...
      const ElementId<Dim> element_id{
          block, element_index_to_segment_id(
                     gsl::at(refinements_by_block, block), element_index)};
...
void check_element_distribution_uniformity(
  ...
  const size_t number_of_elements =
      std::accumulate(refinements_by_block.begin(), refinements_by_block.end(),
                      0_st, [](size_t lhs, const std::array<size_t, Dim>& rhs) {
...
void check_element_distribution_cohesion(
  ...
  for (size_t block = 0; block < proc_map.size(); ++block) {
    ...
    for (size_t i = 1; i < Dim; ++i) {
      strides.at(i) =
          gsl::at(strides, i - 1) *
          two_to_the(gsl::at(gsl::at(refinements_by_block, block), i - 1));
    }
    ...
    for (size_t start_element = 0;
      ...
      if (not seen.at(start_element)) {
        ...
        auto insert_adjacent_unseen = [&next_elements, &current_proc, &proc_map,
                                       &refinements_by_block, &strides, &seen,
          ...
          for (size_t i = 0; i < Dim; ++i) {
            ...
            if (index_in_dim + 1 <
                    two_to_the(
                        gsl::at(gsl::at(refinements_by_block, block), i)) and
                not seen.at(index + gsl::at(strides, i)) and
                gsl::at(gsl::at(proc_map, block),
                        index + gsl::at(strides, i)) == current_proc) {
              next_elements.push_back(index + gsl::at(strides, i));
  ...
  // this check assumes that only one block is a different size; a more involved
  // calculation is needed if the calling code becomes more general.
  for (const auto& blocks : block_set_per_proc) {
    CHECK(blocks.size() <=
          static_cast<size_t>(
              std::ceil(static_cast<double>(refinements_by_block.size()) /
                        number_of_procs) +
...
void test_single_block_domain() noexcept {
  std::vector<std::array<size_t, Dim>> refinements_by_block;
  refinements_by_block.emplace_back();
  for (size_t i = 0; i < Dim; ++i) {
    refinements_by_block.at(0).at(i) = 3;
  }

  std::vector<std::vector<size_t>> proc_map =
      make_proc_map_for_domain(1, two_to_the(Dim), refinements_by_block);
  ...
  for (size_t cube_index = 0; cube_index < two_to_the(Dim); ++cube_index) {
    ...
    for (size_t i = 0; i < Dim; ++i) {
      reference_segment_id.at(i) =
          SegmentId{gsl::at(gsl::at(refinements_by_block, 0), i),
    ...
    for (size_t element_within_cube = 0;
         element_within_cube <
         number_of_elements_in_block(gsl::at(refinements_by_block, 0)) /
             two_to_the(Dim);
         ++element_within_cube) {
  ...
  proc_map = make_proc_map_for_domain(1, 5, refinements_by_block);
  check_element_distribution_uniformity(proc_map, 5, refinements_by_block);
  check_element_distribution_cohesion(proc_map, 5, refinements_by_block);
}
...
void general_test(const size_t number_of_blocks, const size_t number_of_procs,
  ...
  for (size_t i = 0; i < number_of_blocks; ++i) {
    ...
    for (size_t j = 0; j < Dim; ++j) {
      refinements_by_block.at(i).at(j) = uneven_domain ? j : 1;
    }
  }
  if (uneven_domain) {
    for (size_t j = 0; j < Dim; ++j) {
      refinements_by_block.at(0).at(j) = j % 2 == 0 ? 1 : 2;
    }
  }
  const std::vector<std::vector<size_t>> proc_map = make_proc_map_for_domain(
      number_of_blocks, number_of_procs, refinements_by_block);
  check_element_distribution_uniformity(proc_map, number_of_procs,
                                        refinements_by_block);
  check_element_distribution_cohesion(proc_map, number_of_procs,
                                      refinements_by_block, uneven_domain);
}
