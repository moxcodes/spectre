...
void test_derivative_via_transforms() noexcept {
  ...
  detail::compute_derivative_coefficients<DerivativeKind, Spin>(
      l_max, number_of_radial_points, make_not_null(&derivative_modes),
      make_not_null(&transform_modes));
...
// optimization. This calls the utility for two derivatives, `DerivativeKind1`
// and `DerivativeKind2`, each applied to two scalars of spin-weight `Spin1` and
// `Spin2`. The result is compared to analytical forms obtained by multiplying
...
void test_compute_swsh_derivatives() noexcept {
  ...
  using input_tag_list = tmpl::list<TestTag<0, Spin1>, TestTag<1, Spin2>>;
  using derivative_tag_list =
      tmpl::list<Tags::Derivative<TestTag<0, Spin1>, DerivativeKind1>,
                 Tags::Derivative<TestTag<1, Spin2>, DerivativeKind1>,
                 Tags::Derivative<TestTag<0, Spin1>, DerivativeKind2>,
                 Tags::Derivative<TestTag<1, Spin2>, DerivativeKind2>>;
  ...
  ComplexModalVector expected_modes_spin_2{
      number_of_radial_points * size_of_libsharp_coefficient_vector(l_max)};
  TestHelpers::generate_swsh_modes<Spin2>(
      make_not_null(&expected_modes_spin_2), make_not_null(&gen),
      make_not_null(&coefficient_distribution), number_of_radial_points, l_max);

  const auto coefficients_to_analytic_collocation = [&l_max](
      auto computed_collocation, ComplexModalVector& modes) noexcept {
    ...
    TestHelpers::swsh_collocation_from_coefficients_and_basis_func<
        ...
        number_of_radial_points, TestHelpers::spin_weighted_spherical_harmonic);
  };
  // Put the collocation information derived from the generated modes in the
  // DataBox
  db::mutate<TestTag<0, Spin1>>(make_not_null(&box),
                                coefficients_to_analytic_collocation,
                                expected_modes_spin_1);
  db::mutate<TestTag<1, Spin2>>(make_not_null(&box),
                                coefficients_to_analytic_collocation,
                                expected_modes_spin_2);
  ...
  ComplexDataVector expected_derivative_1_collocation_spin_1{
      number_of_radial_points * number_of_swsh_collocation_points(l_max)};
  ComplexDataVector expected_derivative_1_collocation_spin_2{
      number_of_radial_points * number_of_swsh_collocation_points(l_max)};
  ComplexDataVector expected_derivative_2_collocation_spin_1{
      number_of_radial_points * number_of_swsh_collocation_points(l_max)};
  ComplexDataVector expected_derivative_2_collocation_spin_2{
      number_of_radial_points * number_of_swsh_collocation_points(l_max)};
  ...
  TestHelpers::swsh_collocation_from_coefficients_and_basis_func<
      ...
      make_not_null(&expected_derivative_1_collocation_spin_2),
      expected_modes_spin_2, l_max, number_of_radial_points,
      TestHelpers::derivative_of_spin_weighted_spherical_harmonic<
          DerivativeKind1>);

  TestHelpers::swsh_collocation_from_coefficients_and_basis_func<
      Spin1, Representation>(
      make_not_null(&expected_derivative_2_collocation_spin_1),
      expected_modes_spin_1, l_max, number_of_radial_points,
      TestHelpers::derivative_of_spin_weighted_spherical_harmonic<
          DerivativeKind2>);
  ...
  TestHelpers::swsh_collocation_from_coefficients_and_basis_func<
      Spin2, Representation>(
      make_not_null(&expected_derivative_2_collocation_spin_2),
      expected_modes_spin_2, l_max, number_of_radial_points,
      TestHelpers::derivative_of_spin_weighted_spherical_harmonic<
          DerivativeKind2>);

  // the actual derivative call
  db::mutate_apply<SwshDerivatives<derivative_tag_list, Representation>>(
      make_not_null(&box));
          ...
          .scale(1.0);
  {
    ...
    CHECK_ITERABLE_CUSTOM_APPROX(
        expected_modes_spin_1,
        get(db::get<Tags::SwshTransform<TestTag<0, Spin1>>>(box)).data(),
        swsh_approx);
    CHECK_ITERABLE_CUSTOM_APPROX(
        expected_modes_spin_2,
        get(db::get<Tags::SwshTransform<TestTag<1, Spin2>>>(box)).data(),
        swsh_approx);
  }
  {
    ...
    CHECK_ITERABLE_CUSTOM_APPROX(
        ...
        get(db::get<Tags::Derivative<TestTag<0, Spin1>, DerivativeKind1>>(box))
            .data(),
        swsh_approx);
    CHECK_ITERABLE_CUSTOM_APPROX(
        expected_derivative_1_collocation_spin_2,
        get(db::get<Tags::Derivative<TestTag<1, Spin2>, DerivativeKind1>>(box))
            .data(),
        swsh_approx);
    CHECK_ITERABLE_CUSTOM_APPROX(
        expected_derivative_2_collocation_spin_1,
        get(db::get<Tags::Derivative<TestTag<0, Spin1>, DerivativeKind2>>(box))
            .data(),
        swsh_approx);
    CHECK_ITERABLE_CUSTOM_APPROX(
        expected_derivative_2_collocation_spin_2,
        get(db::get<Tags::Derivative<TestTag<1, Spin2>, DerivativeKind2>>(box))
            .data(),
        swsh_approx);
  }
  {
