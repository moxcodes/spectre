 ...
 * are needed for either deregistering when an element is being eliminated from
 * a core or registering when an element is added to a core outside of
 * initialization.
 * The use of separate functions is necessary to provide an interface usable
 * outside of iterable actions, e.g. in specialized `pup` functions.
 */
