 ...
 * details on these parameters). Next, a sharp angular filter is applied which
 * simply sets to zero all `l > limit_l` modes. For comparisons with SpEC CCE,
 * `exponential_half_power` of 8, `exponential_alpha` of 108, and `limit_l` of
 * `l_max - 3` should be used.
 ...
 * surface.
 *
 * \details A sharp angular filter is applied which simply sets to
 * zero all `l > limit_l` modes. For comparisons with SpEC CCE, `limit_l` of
 * `l_max - 3` should be used.
 */
template <int Spin>
