...
ComplexDiagonalModalOperator derivative_factors(size_t l_max) noexcept {
  ...
  for (size_t i = 0; i < number_of_radial_points; ++i) {
    ...
    const ComplexModalVector input_mode_view{
        pre_derivative_modes->data().data() +
            i * size_of_libsharp_coefficient_vector(l_max),
        size_of_libsharp_coefficient_vector(l_max)};
    output_mode_view = input_mode_view * modal_derivative_operator;
  }
}
}  // namespace detail
...
swsh_derivative(
  ...
  auto result =
      SpinWeighted<ComplexDataVector, Tags::derivative_spin_weight<DerivKind> +
                                          Spin>{to_derive.size()};
  swsh_derivatives<tmpl::list<DerivKind>, Representation>(
      l_max, number_of_radial_points, make_not_null(&result), to_derive);
  return result;
}
...
#define DERIVKIND_AND_SPIN_INSTANTIATION(r, data)                            \
  namespace detail {                                                         \
  template void                                                              \
  compute_derivative_coefficients<GET_DERIVKIND(data), GET_SPIN(data)>(      \
      const size_t l_max, const size_t number_of_radial_points,              \
...
#define FULL_DERIVATIVE_INSTANTIATION(r, data)                              \
  template SpinWeighted<ComplexDataVector,                                  \
                        Tags::derivative_spin_weight<GET_DERIVKIND(data)> + \
                            GET_SPIN(data)>                                 \
  swsh_derivative<GET_DERIVKIND(data), GET_REPRESENTATION(data),            \
                  GET_SPIN(data)>(                                          \
      const size_t l_max, const size_t number_of_radial_points,             \
