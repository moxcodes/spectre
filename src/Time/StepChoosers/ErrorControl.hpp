...
/// Suggests a step size based on a target absolute and/or relative error
/// measure.
///
/// The suggested step is calculated via a simple specialization of the scheme
/// suggested in \cite{Harrier1993}. We first compute the aggregated error
...
/// \f$i\f$.
///
/// The new step has size:
///
/// \f{align*}{
/// h_{\text{new}} = h \cdot \min(F_{\text{max}}, \max(F_{\text{min}}),
/// \cdot \frac{F_{\text{safety}}{E^{1/q}}}),
/// \f}
///
...
/// within the target error. \f$q\f$ is the order of the stepper.
/// Intuitively, we should change the step less drastically for a higher order
/// stepper.
///
 ...
 public:
  ...
  std::pair<double, bool> operator()(
    ...
    if (not stepper_error_updated) {
      return std::make_pair(previous_step, true);
    }
  ...
  double error_calc_impl(const EvolvedType& values,
    ...
    if constexpr (tt::is_iterable_v<std::remove_cv_t<EvolvedType>>) {
      double result = 0.0;
...
struct IsUsingTimeSteppingErrorControl : IsUsingTimeSteppingErrorControlBase,
  ...
  static bool create_from_options(
    ...
    tmpl::for_each<
        ...
        [&is_adaptive, &step_choosers](auto step_chooser_type_v) noexcept {
