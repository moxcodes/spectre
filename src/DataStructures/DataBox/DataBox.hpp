 ...
 * "unsafely" extracted in this manner, so this should be done only when no
 * alternatives are possible and when it is known that no compute or reference
 * tags will access the unsafely extracted tag.
 */
template <typename... MutateTags, typename TagList, typename Invokable,
          typename... Args>
  ...
  using first_compute_items_to_reset = tmpl::remove_duplicates<
      tmpl::transform<tmpl::filter<typename DataBox<TagList>::edge_list,
                                   tmpl::bind<tmpl::list_contains,
                                              ...
                                              tmpl::get_source<tmpl::_1>>>,
                      tmpl::get_destination<tmpl::_1>>>;
  if constexpr (not std::is_same_v<decltype(invokable(
                                       make_not_null(
                                           &box->template get_deferred<
                                                   detail::first_matching_tag<
                                                       TagList, MutateTags>>()
                                                .mutate())...,
