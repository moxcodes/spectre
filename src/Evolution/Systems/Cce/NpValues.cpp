...
void weyl_psi0_impl(
  ...
  auto dy_beta = 0.125 * one_minus_y *
                 (dy_j * conj(dy_j) -
                  0.25 * square(bondi_j * conj(dy_j) + conj(bondi_j) * dy_j) /
                      square(bondi_k));
  *psi_0 =
      pow<4>(one_minus_y) * 0.0625 / square(bondi_r) *
      ((1.0 + bondi_k) * dy_beta * dy_j / bondi_k -
       square(bondi_j) * dy_beta * conj(dy_j) / (bondi_k * (1.0 + bondi_k)) +
       0.5 * bondi_j * (1.0 + square(bondi_k)) * dy_j * conj(dy_j) /
           pow<3>(bondi_k) -
       0.5 * (1.0 + bondi_k) * dy_dy_j / bondi_k +
       0.5 * square(bondi_j) * conj(dy_dy_j) / (bondi_k * (1.0 + bondi_k)) -
       0.25 * bondi_j *
