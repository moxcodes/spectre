...
struct OnDemandInputsForSwshJacobianImpl<
  ...
  SPECTRE_ALWAYS_INLINE decltype(auto) operator()(
      const DataBoxType& box) noexcept {
...
/// situations in which it is possible that a repeated computation is more
/// desirable than storing a value in the `DataBox` (e.g. for conjugation).
template <typename Tag>
...
// this mutation needs access to several of the full variables from the
// computation in order to make intelligent decisions (via the Get... structs
// above) about whether to retrieve certain inputs from the Variables or to
// obtain them via expression template formulas if they are cheap enough to
// compute on demand.

/*!
 * \brief Performs a mutation to a spin-weighted spherical harmonic derivative
 * value from the numerical coordinate (i.e. the spin-weighted derivative at
 * fixed \f$y\f$) to the Bondi coordinates (i.e. the spin-weighted derivative at
 * fixed \f$r\f$), inplace to the requested tag. This should be performed only
 * once for each derivative evaluation for each tag, as a repeated inplace
 * evaluation will compound and result in incorrect values in the  `DataBox`.
 *
 * \details This is compatible with acting as a mutation in `db::mutate_apply`.
 */
 ...
 * \details The implemented equation is:
 *
 * \f[ \bar{\eth} F = \bar{\eth}^\prime F - (1 - y) \frac{\bar{\eth} R}{R}
 * \partial_y F, \f]
 *
 ...
 * \details The implemented equation is:
 *
 * \f[ \eth \bar{\eth} F = \eth^\prime \bar{\eth}^\prime F - \frac{\eth R
 * \bar{\eth} R}{R^2} (1 - y)^2 \partial_y^2 F - (1 - y)\left(\frac{\eth R}{R}
 * \bar{\eth} \partial_y F + \frac{\bar{\eth} R}{R} \eth \partial_y F +
 * \frac{\eth \ethbar R}{R} \partial_y F\right), \f]
 *
 ...
 * \details The implemented equation is:
 *
 * \f[ \bar{\eth} \eth F = \bar{\eth}^\prime \eth^\prime F - \frac{\eth R
 * \bar{\eth} R}{R^2} (1 - y)^2 \partial_y^2 F - (1 - y)\left(\frac{\eth R}{R}
 * \bar{\eth} \partial_y F + \frac{\bar{\eth} R}{R} \eth \partial_y F +
 * \frac{\eth \ethbar R}{R} \partial_y F\right), \f]
 *
 ...
 * \details The implemented equation is:
 *
 * \f[ \eth \eth F = \eth^\prime \eth^\prime F
 ...
 * \details The implemented equation is:
 *
 * \f[ \bar{\eth} \bar{\eth} F = \bar{\eth}^\prime \bar{\eth}^\prime F
...
// rule applies)
template <typename DerivativeTag, typename DataBoxType,
          typename... OnDemandTags>
void apply_swsh_jacobian_helper(const gsl::not_null<DataBoxType*> box,
 ...
 * `BondiValueTag` which are spin-weighted angular derivatives. This function is
 * called on the `DataBox` holding the relevant CCE data during each
 * hypersurface integration step, after evaluating
 * `mutate_all_pre_swsh_derivative_for_tag<BondiValueTag>` and before evaluating
 * `ComputeBondiIntegrand<BondiValueTag>`.
 *
 * \details Provided a `DataBox` with the appropriate tags (including
 * `all_pre_swsh_derivative_tags`, `all_swsh_derivative_tags`,
 * `all_transform_buffer_tags`,  `pre_computation_tags`,
 * `all_pre_swsh_derivative_tags`, and `Spectral::Swsh::Tags::LMax`), this
 * function will apply all of the necessary mutations to update
 * `all_swsh_derivatives_for_tag<BondiValueTag>` to their correct values for the
 * current values of the remaining (input) tags.
 * \note The spin-weighted derivatives (tags `all_swsh_derivative_tags`), the
 * transform buffers (tags `all_transform_buffer_tags`), and the values to
 * spin-weighted differentiate (tags `all_pre_swsh_derivative_tags`) must be in
 * `Variables`, and likely different `Variables` associated with the respective
 * tags provided as explicit  template arguments to this function
 * `SwshDerivativeVariablesTag`, `CoefficientBufferVariablesTag`,
 * `PreSwshDerivativeVariablesTag`. The requirement of storage in `Variables` is
 * necessary to pass on to the efficient evaluation of
 * `Spectral::Swsh::compute_swsh_derivatives` which computes the derivatives in
 * clusters of like spin-weights to save on repeated computations.
 */
...
void mutate_all_swsh_derivatives_for_tag(
  ...
  db::mutate_apply<Spectral::Swsh::SwshDerivatives<
      swsh_derivative_tags_to_compute_for<BondiValueTag>>>(box);
  tmpl::for_each<swsh_derivative_tags_to_compute_for<BondiValueTag>>(
      [&box](auto x) {
  ...
  db::mutate_apply<Spectral::Swsh::SwshDerivatives<
      second_swsh_derivative_tags_to_compute_for<BondiValueTag>>>(box);
  tmpl::for_each<second_swsh_derivative_tags_to_compute_for<BondiValueTag>>(
      [&box](auto x) {
        using derivative_tag = typename decltype(x)::type;
        detail::apply_swsh_jacobian_helper<derivative_tag>(
            box, typename ApplySwshJacobianInplace<
