 ...
 * reached.
 *
 * \warnings the global sync points _must_ be triggered often enough to ensure
...
struct CheckpointAndExitAfterWallclock
  ...
  CheckpointAndExitAfterWallclock() noexcept {
  ...
  CheckpointAndExitAfterWallclock(const std::optional<double> wallclock_hours,
    ...
    if (wallclock_hours.has_value() and wallclock_hours.value() < 0.0) {
      PARSE_ERROR(context, "Must give a positive time in hours");
    }
    time_at_start_ = std::chrono::system_clock::now();
  }
  ...
  template <typename ParallelComponent, typename LocalMetavariables,
            typename ArrayIndex>
  ...
  arbitrate_phase_change_impl(
    ...
    const double elapsed_hours = [this]() noexcept {
      const auto now = std::chrono::system_clock::now();
      const auto delta = now - time_at_start_;
    ...
    if (restart_phase.has_value()) {
      ...
      // This branch is the post-checkpoint call to arbitrate_phase_change.
      // Depending on the time elapsed so far in this run, next phase is...
      // - Exit, if the time is large (this is checkpoint -> exit)
      // - restart_phase, if the time is small (this is restarting from a chkpt)
  ...
  void pup(PUP::er& p) noexcept override {
    // Do not checkpoint time_at_start_: we reset this value on restarts
